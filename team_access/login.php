<?php
	...
	if(isset($_POST['login'])){
		...
		$checkUser = $mysqlObj->mysqlQuery("SELECT id,name,suser_id,pass,userType,api_access_key FROM admin WHERE suser_id='".$post['user']."' AND pass='".$hash_pass."' ");
		//echo "SELECT id,name,suser_id,pass FROM admin WHERE suser_id='".$post['user']."' AND pass='".$hash_pass."' ";
		if($checkUser->rowCount()>0){
		
			$user = $checkUser->fetch(PDO::FETCH_ASSOC);
			
			if( $user['pass']==$hash_pass && $user['suser_id']==$post['user'] ){
			
				$_SESSION[_session_username_]   =   $user['name'];
				$_SESSION[_session_userid_] =   $user['id'];
				$_SESSION[_session_usertype_] =   $user['userType'];
				$_SESSION[_session_apiKey_] =   $user['api_access_key'];
...
<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">	
	...
	<div class="col-md-12">
	<input type="username" required class="form-control" placeholder="user_name" name="user" value="dnetp_admin"/>
	</div>
	</div>
	<div class="form-group">
	<div class="col-md-12">
	<input type="password" required class="form-control" placeholder="Password" name="password" value="(@DMT!@#P@isa)" />
	</div>
	</div>
	<div class="form-group">
	<div class="col-md-6">
