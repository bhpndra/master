...
if(!empty($post['bankitOutletId']) && !empty($post['userassign']) && !empty($post['credit'])){
	...
	$status = $mysqlClass->updateData('bankit_outlet_kyc',$update,"WHERE id='$oid'");
	if ($status) {
										...
										<div class="form-group col-md-4">
											...
											<div>
												<select name="bankitOutletId" class='form-control select2' required>
													...
													<?php
														$utypes = $mysqlClass->fetchAllData("bankit_outlet_kyc","outlet_id,id,mobile,fname", " where `user_id` =''");
														
														foreach($utypes as $ut){
															
													?>
														<option value="<?=$ut['id']?>"><?=$ut['outlet_id']?> (<?=$ut['mobile']?>) - <?=$ut['fname']?></option>
													<?php } ?>
												</select>
											</div>
										</div>
										...
										<div class="form-group col-md-4">
											...
											<div>
												<select name="userassign" class='form-control select2' required>
													...
													<?php
														$add_cust = $mysqlClass->fetchAllData("add_cust","id,mobile,user,cname", " where id not in(SELECT user_id FROM `bankit_outlet_kyc` WHERE user_id!='')");
