<?php
	...
	if ( isset($post['rowId']) AND $post['tranId'] != '' ) {
	   ...
	   //Opening Balance
			$selqrO = "SELECT `balance` FROM `retailer_trans` WHERE id < (SELECT id FROM retailer_trans WHERE `transaction_id`='".$post['tranId']."') && `retailer_id`='".$post['userID']."' ORDER BY id DESC LIMIT 1 ";			
