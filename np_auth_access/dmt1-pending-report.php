...
$sql = "SELECT d.*  FROM `dmt_info` as d WHERE 1 ";	
			...
			?>
									<tr>
										...
										<td id="balDetails<?php echo $rows["dmt_id"]; ?>">
											<a href="javascript:void(0)" onclick="getBalanceDetails('<?php echo $rows["topup_trid"]; ?>','<?php echo $rows["user_type"]; ?>','<?php echo $rows["agent_id"]; ?>','<?php echo $rows["dmt_id"]; ?>')">Get Balance Details</a>
										</td>
										<td>
										...
										<td id="tcol<?php echo $rows["dmt_id"]; ?>">
											...
											<a href="javascript:void(0)" onclick="change_status('<?php echo $rows["dmt_id"]; ?>','<?php echo $rows["agent_id"]; ?>')" class="btn btn-success btn-xs statusmsg" style="margin-bottom:5px;">Change Status</a>
											<?php } ?>
										</td>
...
<script>
	$(document).ready(function(){
		...
		function change_status(val,agent_id){
			...
			function(){
			  ...
			  if(sel!=''){
				  ...
				  $.ajax({
						...
						data: {'status':sel,'id':val, 'agent_id':agent_id},
						success: function (response) {
