...
if (isset($_POST['userType']) && isset($_POST['user']) && isset($_POST['bankname']) && isset($_POST['amount'])) {
	...
	/* echo $credit_user_id ." - ". $retailerid;
	echo "<br/>SELECT `balance`,`ret_dest_wl_admin_id`,`date_created` FROM `admin_trans` WHERE `admin_id`='" . $_SESSION[_session_userid_] . "' ORDER BY `trans_id` DESC LIMIT 1 "; */
			...
			if ( $last_tran ) {
				...
				if ($mysqlClass->insertData('admin_trans', $value) > 0) {
					...
					swal({
							...
							function(){
							  window.location = "credit-to-user_1.php";
							});					
					</script>';
					//echo "<script>alert('Amount Credited...')</script>";
					//echo "<script>window.location = 'credit-to-user_1.php';</script>";
				} else {
										...
										<div class="form-group col-md-4">
											...
											<div>
												<select name="userType" class='form-control' required onchange="selectUserType(this.value)">
													...
													?>
														<option <?=($ut['id']==2)? 'selected' : '';?> value="<?=$ut['id']?>"><?=$ut['name']?></option>
													<?php } ?>
												</select>
											</div>
										</div>
...
<?php
$mysQuery = $mysqlClass->mysqlQuery("SELECT * FROM `add_cust` WHERE wallet_balance = '0' and `id` IN (SELECT `user_id` FROM `add_retailer`)");
...
?>
										...
										<div class="form-group col-md-7" >
											<label>Amount</label>
											<div>
												<input type='text' name='amount' maxlength="7" class='form-control' onkeyup="convertNumberToWords(this.value);" value="0.02" onblur="convertNumberToWords(this.value);" id="amount" placeholder="Amount" >
												<p id="amountinword"></p>
											</div>
										</div>
