...
<?php
	...
	if($filterBy['fBycreateIn']!="" && isset($filterBy['fBycreateIn'])){
		...
		$filter .= " and ( STR_TO_DATE(created_on, '%d/%b/%Y') BETWEEN '$dateFrom' AND '$dateTo' ) ";
	} 
															...
															<div class="form-group col-md-3">
																...
																?>
																	<select class='form-control' name='fByUsertype' >					
																		...
																		<option <?=@$WhiteLabel?> value="White Label">White Label</option>
                                                        ...
                                                        <table class="table table-bordered">
                                                            <thead>
                                                                <tr>
                                                                    ...
                                                                    <th scope="col" width="150px;">Ledger Closing Bal</th>
...
$sql = "SELECT id, name, user, mobile, cname, aeps_balance, wallet_balance, 
			...
			?>
									<tr>
										...
										<td width="250px;">
											...
											<strong>User Type:</strong> <?= $rows['userType']; ?><br/>
										</td>
										<td><?= $rows['aeps_balance']; ?></td>
										<td><?= $rows['wallet_balance']; ?></td>
										<td width="150px;" class="amount">	
											<?php if($rows['userType']=="Retailer"){ 
												$closing = $mysqlObj->mysqlQuery("SELECT balance FROM `retailer_trans` where retailer_id = '".$rows['id']."' ORDER BY `id`  DESC LIMIT 1")->fetch(PDO::FETCH_ASSOC);												
												if($closing['balance']!=$rows['wallet_balance']){
													echo "<span style='color:red'>".$closing['balance']."</span>";
												} else {
													echo $closing['balance'];
												}												
												unset($closing['balance']);
											} 
											?>
											
											<?php if($rows['userType']=="Distributer"){ 
												$closing = $mysqlObj->mysqlQuery("SELECT balance FROM `distributor_trans` where dist_id = '".$rows['id']."' ORDER BY `id`  DESC LIMIT 1")->fetch(PDO::FETCH_ASSOC);
												if($closing['balance']!=$rows['wallet_balance']){
													echo "<span style='color:red'>".$closing['balance']."</span>";
												} else {
													echo $closing['balance'];
											...
											<?php if($rows['userType']=="White Label"){ 
												$closing = $mysqlObj->mysqlQuery("SELECT balance FROM `wl_trans` where wluser_id = '".$rows['id']."' ORDER BY `id`  DESC LIMIT 1")->fetch(PDO::FETCH_ASSOC);
												if($closing['balance']!=$rows['wallet_balance']){
													echo "<span style='color:red'>".$closing['balance']."</span>";
												} else {
													echo $closing['balance'];
