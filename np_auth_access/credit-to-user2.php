...
if (isset($_POST['userType']) && isset($_POST['user']) && isset($_POST['bankname']) && isset($_POST['amount'])) {
	...
	$amount     = $post['amount'];
	$su_avlbal         = $mysqlClass->mysqlQuery("SELECT `by_actual_admin`,`balance`,`ret_dest_wl_admin_id`,`date_created` FROM `admin_trans` WHERE `admin_id`='" . $_SESSION[_session_userid_] . "' and withdrawl!='' and withdrawl!='NULL' ORDER BY `trans_id` DESC LIMIT 1 ")->fetch(PDO::FETCH_ASSOC);
	$superadmin_bal    = $su_avlbal['balance'];
	$by_actual_admin    = $su_avlbal['by_actual_admin'];
	...
	if (($transaction_gap < 120) && $by_actual_admin=='YES') { //300 is the seconds
