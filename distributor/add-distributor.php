...
$isMaster = $mysqlClass->mysqlQuery("SELECT is_master_distributor FROM `add_distributer` WHERE `user_id`='" . $USER_ID . "' ")->fetch(PDO::FETCH_ASSOC);
if($isMaster['is_master_distributor']!=1){
	$previous = ($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : DOMAIN_NAME."distributor/";
	$helpers->redirect_page($previous);
}
			...
			if($que->rowCount()>0){				
				...
				if(!empty($resAAK['api_access_key'])){
					...
					if($last_id>0){
						...
						$userDetailes = "LoginId: DT".$last_id." Password: ".$post['pass']." PIN: ".$post['s_pin'];
