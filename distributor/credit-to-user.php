...
$post = $helpers->clearSlashes($_POST); 
	if(isset($_POST['amount']) && $_POST['amount'] > 0 && isset($_POST['user']) && isset($_POST['userType'])){
		$mysqlClass->mysqlQuery("START TRANSACTION");		
			$wl_avlVbal = $mysqlClass->mysqlQuery("SELECT wallet_balance FROM `add_cust` WHERE `id`='" . $USER_ID . "' ")->fetch(PDO::FETCH_ASSOC);	
			
			$wl_v_bal  = $wl_avlVbal['wallet_balance'];
			$amount	= $post['amount'];
			$cr_UserId = $post['user'];
			...
			if ( $wl_v_bal > $amount ) {
				...
				if(isset($lastid) && $lastid > 0){
					...
					//$mysqlClass->mysqlQuery("UPDATE `add_cust` set wallet_balance = (wallet_balance - $amount) WHERE `id`='" . $USER_ID . "' and admin_id = '".$ADMIN_ID."' ");
				...
				$msg  = $helpers->alert_message("You have insufficient balance to transafer.","alert-danger");
			}
