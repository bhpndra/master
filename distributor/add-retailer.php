...
$limit = $mysqlClass->mysqlQuery("select number_of_child_limi from `add_cust` WHERE `id`='".$USER_ID."' and `admin_id`='".$ADMIN_ID."' and usertype = 'DISTRIBUTOR' ")->fetch(PDO::FETCH_ASSOC);
...
$post = $helpers->clearSlashes($_POST);
	if(isset($_POST['add'])){
		...
		$distId = $post['distributor'];
		
		$distPackage = $mysqlClass->fetchRow("add_cust"," package_id ", " WHERE id = '".$USER_ID."' and usertype = 'DISTRIBUTOR' ");
		$packageID = $distPackage['package_id'];
		...
		if($que->rowCount()>0){				
			...
			if(!empty($resAAK['api_access_key'])){
				...
				$amt_capping = ($post['amt_capping'] > 0) ? $post['amt_capping'] : 100; 
				
				$value = array(
					'min_amt_capping' 		=> $post['amt_capping'],
					'name' 					=> $post['name'],
					'mobile' 				=> $post['mobile'],
				...
				if($last_id>0){
						...
						$mysqlClass->insertData(' add_retailer ', $dValue);
							...
							$updateLimit = $limit['number_of_child_limi']- 1;
							$mysqlClass->updateData(' add_cust ', array('number_of_child_limi'=>$updateLimit), " where id = '".$USER_ID."' ");
						
					$userDetailes = "LoginId: RT".$last_id." Password: ".$post['pass']." PIN: ".$post['s_pin']. " ".DOMAIN_NAME;
					...
					$helpers->redirect_page("all-retailer");
				}
			} else {
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Add Retailer</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
		...
		<div class="row">
          ...
          <div class="card card-success">
            ...
            <div class="card-body row">
              ...
              <div class="form-group col-md-4">
                <label for="inputName">Login Pin (4 Digit)</label>
                <input type="text"  class="form-control" value="" maxlength="4" pattern="[0-9]{4}" name="s_pin" required >
              </div>
              <div class="form-group col-md-4">
              ...
              <div class="form-group col-md-3">
                <label for="inputName">Amt Capping (INR)</label>
                <input type="text"  class="form-control" value="100" name="amt_capping"  maxlength="4" required />
              </div>
              <div class="form-group col-md-3">
