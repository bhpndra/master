...
***************/
	...
	if($_SERVER['REQUEST_METHOD']=="POST" && isset($_POST)){
		...
		if(isset($dateFrom) && isset($dateTo) && !empty($dateFrom) && !empty($dateTo)){
			...
			$filter .= " and (date(payment_date) BETWEEN '$dateFrom' AND '$dateTo') ";
		} else {
		...
		$totalRows = $mysqlClass->countRows(" select a.id from online_payment_request as a where a.user_id = '".$USER_ID."' and a.user_type = 'RETAILER' ".$filter);
		
		$columns = " * ";
		$query = $mysqlClass->mysqlQuery("select ".$columns." from online_payment_request where user_id = '".$USER_ID."' and user_type = 'RETAILER' ".$filter.$orderBy.$pagin );
