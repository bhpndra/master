...
$custOtp = new 	CustomerOtp();
	...
	if($_SERVER['REQUEST_METHOD']=="POST" && isset($_POST)){
		...
		if(isset($post['password']) && isset($post['userid'])){
			...
			$CheckUserDisabled = $mysqlObj->fetchRow("`add_cust`", " id "," where `user`='".$post['userid']."' and `pass`='".$hash_password."' and `status` = 'DISABLED' ");
			if($CheckUserDisabled == 0){
			
			$user = $mysqlObj->fetchRow("`add_cust`"," `user`,`pass`,`security_pin`,`id`,`name`, `email`, `mobile`,  `service_access`,`netpaisa_services`,`wl_id`,`admin_id`,`creator_id`"," where `user`='".$post['userid']."' and `pass`='".$hash_password."' and `status` = 'ENABLED' ");
