...
***************/
	...
	if($_SERVER['REQUEST_METHOD']=="POST" && isset($_POST)){
		...
		$fetch_result_json = $helpers->netpaisa_curl("https://www.netpaisa.com/nps/api/finance/get_insurance_token.php", $request_param);
		$fetch_result = json_decode($fetch_result_json,true); 
		if($fetch_result['status']==1){
			$response['ERROR_CODE'] = 0;
			$response['MESSAGE'] = 'Outlet Found';
			$response['URL'] = $fetch_result['data']['booking_url'];
		} else {
