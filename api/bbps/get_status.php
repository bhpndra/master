...
***************/
	...
	if($_SERVER['REQUEST_METHOD']=="POST" && isset($_POST)){
		...
		$sqlRecharge = "Select a.agent_trid, a.operator_value, a.amount, b.id as userid, b.admin_id, b.wl_id, b.package_id, b.creator_id from billpayment_info as a left join add_cust as b on a.user_id = b.id where a.agent_trid = '".$agent_tranid."' and a.status = 'PENDING' ";
			...
			if(!isset($rechargeDetails['agent_trid'])){
				...
				$network = $rechargeDetails['operator_value'];
		...
		$status_result_json = $helpers->netpaisa_curl($bill_status, $request_param);
		//$status_result_json = '{"status_code":"01","status_msg":"PIF037 : PaymentInformation Tag name and value segemnts not matching"}';
		//$status_result_json = '{"status_code":"00","status_msg":"SUCCESS","data":{"clientid":"R15878980712MG7","transaction_id":"205903546","mobile":"7903881364","amount":"10.00","status":"SUCCESS"}}';
		...
		if(isset($status_result['status_code']) && $status_result['status_code']=='00'){
				...
				$mysqlClass->updateData(" billpayment_info ", $valueRIU, " where agent_trid = '".$tranAgentId."'");
				
				$valueRTTrU = array(
							'comments' 			=> "BBPS Transaction Successful",
							'transaction_id' 	=> $txid
						);
		...
		else if(isset($status_result['status_code']) && $status_result['status_code']=='03'){
			
			$sqlRecharge = "Select a.id, a.deducted_amount, a.status, a.agent_trid, b.id as userid, b.admin_id, b.wl_id from billpayment_info as a left join add_cust as b on a.user_id = b.id where a.agent_trid = '".$agent_tranid."' and a.status != 'FAILED' ";
			$rechargeDetails = $mysqlClass->mysqlQuery($sqlRecharge)->fetch(PDO::FETCH_ASSOC);
			...
			if(isset($checkRefund['id']) && $checkRefund['id'] > 0){
				...
				$mysqlClass->updateData(" billpayment_info ", $valueRIU, " where id = '".$rechInfo_lastid."'");
