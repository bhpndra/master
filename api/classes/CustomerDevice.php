...
# Get second last id of OTP
 ...
 function CheckIfDeviceExist($device_id,$user_id){
	...
	$query = " SELECT * FROM installed_app WHERE user_id='".$user_id."' and status=1 ORDER BY id DESC ";
 	$stmt = $this->db_conn->prepare($query);
 	$stmt->execute();
    ...
    if($stmt->rowCount() == 0){
	...
	else if($stmt->rowCount() == 1 && $result[0]['device_id'] == $device_id  ) {		
		return true;
	}
 	...
 	public function IsFromValidDistanceArea( $device_id,$user_id,$latitudeFrom,$longitudeFrom ){
		...
		if($stmt->rowCount() == 0) {
			...
			$query = " SELECT * FROM installed_app WHERE device_id='".$device_id."' and status=1  ORDER BY id DESC ";
			$stmt = $this->db_conn->prepare($query);
			$stmt->execute();
			$result = $stmt->fetchAll(PDO::FETCH_ASSOC);
			
			if($stmt->rowCount() == 1 && empty($result[0]['user_id'])  ) {
				$latitudeTo = $result[0]['geo_location_lat'];
				$longitudeTo = $result[0]['geo_location_long'];
			}
		...
		if( !empty($latitudeTo) && !empty($longitudeTo) ) {		
		  ...
		  $DistanceDiffInMeter =  $angle * $earthRadius;
		  
			  if( $DistanceDiffInMeter>50000 ){
				  return false;
			  }else{
				  return true;
			  }
