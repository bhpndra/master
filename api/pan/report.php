...
***************/
	...
	if($_SERVER['REQUEST_METHOD']=="POST" && isset($_POST)){
		...
		//$filter .= (isset($post['status']) && $post['status'] != '') ? " and status = '".$post['status']."'" : '';
		...
		if(isset($dateFrom) && isset($dateTo) && !empty($dateFrom) && !empty($dateTo)){
			...
			if($diff > 30){
				$helpers->errorResponse('You can fetch only 30 Days pan card history.');
			}
		...
		//$filter .= (isset($post['search']) && $post['search'] != '') ? " and (concat_ws(' ',transaction_id,agent_trid,mobile) like '%".$post['search']."%') " : '';
		$orderBy = " order by `id` DESC";
		/////////////////// End //////////////////
		
		$columns = " name, user_name, mobile, alternate_mobile, email, dob, city, state, pin, address, pan_no,aadhar_no,status,create_date";
		//echo "SELECT ".$fields." FROM ".$table." ".$condition;
		$res = $mysqlClass->fetchAllData('pan_card_details', $columns, " where user_id = '".$USER_ID."' ".$filter.$orderBy.$pagin );
		...
		$response['services_status'] = $StatusArray;
		...
		if(count($res)>0){
			...
			$response['MESSAGE'] = 'Pan card Report Not Found';
		}
