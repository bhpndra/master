...
***************/
	...
	if($_SERVER['REQUEST_METHOD']=="POST" && isset($_POST)){
		...
		$res = $mysqlClass->fetchAllData('recharge_info', $columns, " where user_id = '".$USER_ID."' ".$filter.$orderBy.$pagin );
		$networks = $mysqlClass->fetchAllData('network', ' np_operator_code, operator_name' , "  where operator_type in ('PREPAID','POSTPAID','DTH') ");
		foreach($networks as $k=>$v){
			$operator[$v['np_operator_code']] = $v['operator_name'];
		}
		if(count($res)>0){
			$response['ERROR_CODE'] = 0;
			$response['MESSAGE'] = 'Report Found';
			$response['DATA'] = $res;
			$response['NETWORKS'] = $operator;
		} else {
