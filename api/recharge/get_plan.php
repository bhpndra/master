...
***************/
	...
	if($_SERVER['REQUEST_METHOD']=="POST" && isset($_POST)){
		...
		//// Call netpaisa_curl ////
		$request_param = ['api_access_key' => $api_access_key, 'operator_code'=> $operator_code, 'circle'=> $circle_code];

		$circle_result_json = $helpers->netpaisa_curl($netpaisa_recharge_plan, $request_param);
		$circle_result = json_decode($circle_result_json, true);
		
		if($circle_result['status_code']=='00'){
			$response['ERROR_CODE'] = 0;
			$response['MESSAGE'] = 'SUCCESS';
			$response['PLANS'] = $circle_result['plans'];
		} else {
			$response['ERROR_CODE'] = 1;
			$response['MESSAGE'] = $circle_result['status_msg'];
		}
		/////////////////// End //////////////////
