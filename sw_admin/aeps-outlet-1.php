...
$users = $mysqlClass->mysqlQuery("SELECT b.name,b.user,b.mobile as r_mob, a.* FROM `outlet_kyc` as a left join add_cust as b on a.user_id = b.id WHERE b.admin_id = '".$ADMIN_ID."' and b.usertype = 'RETAILER' and b.wl_id = '".$WL_ID."'");
...
?>  
					<tr>
					  ...
					  <td><?=$us['address']?></td>
					  <td><?=$us['pincode']?></td>					  
					  <td><?=($us['outlet_kyc']==1)? 'SUCCESS' : 'PENDING'?></td>
