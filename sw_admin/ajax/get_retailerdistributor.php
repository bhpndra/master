...
$USER_ID = $resVT['DATA']['USER_ID'];
	...
	if($_POST['usertype']=="RETAILER")
	{
		...
		$mysQuery = $mysqlClass->mysqlQuery("SELECT cname,name,mobile,id FROM `add_cust` WHERE 1 and `wl_id`='".$WL_ID."' and admin_id = '".$ADMIN_ID."' and usertype = 'RETAILER' ");
	...
	if($_POST['usertype']=="DISTRIBUTOR")
	{
		...
		//@@all retailer list to pay balance@@
		//===================================================
		$mysQuery = $mysqlClass->mysqlQuery("SELECT cname,name,mobile,id FROM `add_cust` WHERE 1 and `wl_id`='".$WL_ID."' and admin_id = '".$ADMIN_ID."' and usertype = 'DISTRIBUTOR' ");
		while ($row = $mysQuery->fetch(PDO::FETCH_ASSOC))
		{
			echo "<option value='" . $row['id'] . "'>" . $row['name'] . " - " . $row['cname'] ." (".$row['mobile']. ")"."</option>";
		}	
