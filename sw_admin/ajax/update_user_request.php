...
if(isset($post['status']) &&  isset($post['id'])){	
	$FR_status = $mysqlClass->mysqlQuery("SELECT status FROM `user_registration_request` WHERE `id`='".$post['id']."' and `wl_id`='".$WL_ID."' ")->fetch(PDO::FETCH_ASSOC);
	if($FR_status!="SUCCESS"){
			$data = array("status" => $post['status'] );
			$rowU = $mysqlClass->updateData(" user_registration_request ", $data , " WHERE `id`='".$post['id']."' and `wl_id`='".$WL_ID."'  " );
			if($rowU > 0){
