...
$urlST = BASE_URL."/api/check-software-type.php";
$responseSWDetails = api_curl($urlST,$post_fields,$headerArray);
$resSWDetails = json_decode($responseSWDetails,true);
if($resSWDetails['ERROR_CODE']==0 && $resSWDetails['SOFTWARE_TYPE']=='B2B'){} else {
	header("location:../logout.php"); die();
}
...
<?php if(!empty($siteDetails['color_code'])){ ?>
<style>
...
.card-primary:not(.card-outline) > .card-header{
	background-color: <?=$siteDetails['color_code']?>;
	color:#fff;
}
