...
$post = $helpers->clearSlashes($_POST);
	if(isset($_POST['add'])){
		...
		$distPackage = $mysqlClass->fetchRow("add_cust"," package_id ", " WHERE id = '".$distId."' and usertype = 'DISTRIBUTOR' ");
          ...
          <div class="card card-success">
            ...
            <div class="card-body row">
              ...
              <div class="form-group col-md-6">
                ...
                <select name="distributor" class='form-control select2' required>
					<option value="">Select Distributor</option>
					<?php
						$utypes = $mysqlClass->fetchAllData("add_cust","name,id,user,mobile", " WHERE `creator_id` = '".$USER_ID."' and admin_id = '".$ADMIN_ID."' and wl_id = '".$WL_ID."' and created_by = 'WL'");
						foreach($utypes as $ut){
					?>
						<option value="<?=$ut['id']?>"><?=$ut['name']?> - <?=$ut['mobile']?> - <?=$ut['user']?></option>
					<?php } ?>
				</select>
              </div>
            </div>
