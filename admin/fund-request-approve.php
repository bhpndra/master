...
<div class="wrapper">
  ...
  <nav class="main-header navbar navbar-expand navbar-white navbar-light" style="margin-left: 0px;">
    ...
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item dropdown">
        <a class="nav-link" data-toggle="dropdown" href="#">
          <i class="fas fa-fingerprint"></i> AEPS
...
if(!isset($get['uid']) || !isset($get['t']) || !isset($get['amt']) || !isset($get['id'])){
	echo '<script>self.close();</script>';
} else {
	$FR_status = $mysqlClass->mysqlQuery("SELECT status FROM `payment` WHERE `id`='".$get['id']."' and `user_id`='".$cr_UserId."' and `admin_id`='".$ADMIN_ID."' ")->fetch(PDO::FETCH_ASSOC);
	...
	if(isset($_POST['amount']) && $_POST['amount'] > 0 && isset($_POST['user']) && isset($_POST['userType'])){
		$mysqlClass->mysqlQuery("START TRANSACTION");		
			...
			if ( $wl_v_bal > $amount ) {
				...
				$rowU = $mysqlClass->updateData(" payment ", array("status"=>"SUCCESS") , " WHERE `id`='".$get['id']."' and `user_id`='".$cr_UserId."' and `admin_id`='".$ADMIN_ID."' " );
			...
			if($rowU > 0){
				...
				$msg  = $helpers->alert_message("Something Wrong!","alert-danger");
			}
            ...
            <div class="card-body row">
			  <div class="form-group col-md-3">
                ...
                <select name="userType" id="userType" onchange="selectUserType(this.value)" class='form-control' required>
					...
					<option <?=($_GET['t']=="RETAILER") ? "SELECTED" : "DISABLED" ?> value="RETAILER">Retailer</option>
					<option <?=($_GET['t']=="DISTRIBUTOR") ? "SELECTED" : "DISABLED" ?> value="DISTRIBUTOR">Distributor</option>
				</select>
              </div>
              ...
              <div class="form-group col-md-6">
                ...
                <input type="text"  class="form-control" value="<?=$_GET['amt']?>" name="amount" id="amount" maxlength="6" onkeyup="convertNumberToWords(this.value)" onblur="convertNumberToWords(this.value);" required />
...
$(document).ready(function(){
	...
	function selectUserType(val){
		//alert(val);
		var retailer = "<?=base64_decode($_GET['uid'])?>";
		$.ajax({
            ...
            success: function (response) {
				...
				$("#user option").each(function() {
					var ret = $(this).val(); //alert(ret + " " + retailer);
					if(ret==retailer){					
	...
	function load_function(){		
		var users = selectUserType('<?=$_GET['t']?>');		
	}
	window.load = load_function();
