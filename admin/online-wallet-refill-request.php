...
$filter = '';
	...
	if(@$filterBy['dateFrom']=="" && @$filterBy['dateTo']==""){
		...
		$dateTo   = date("Y-m-d");
		$filter  .= " and DATE(a.payment_date) BETWEEN '$dateFrom' AND '$dateTo' ";
	} else{
		$filter .= " and DATE(a.payment_date) BETWEEN '".$filterBy['dateFrom']."' AND '".$filterBy['dateTo']."' ";
...
$sql = " select  a.transaction_id, a.id, a.user_id, a.amount, a.payment_date, a.status, a.user_type, b.name, b.user, b.mobile
		from online_payment_request as a
...
function update_status(e,id,uid,type,amount){ 
	...
	if(confirm("Are you sure to update status ?")){
		if(status == 'SUCCESS'){
			var url = '<?=DOMAIN_NAME?>admin/fund-request-approve?uid=' + uid + '&t=' + type + '&amt=' + amount + "&id=" + id + "&r=pg" ;
			...
			$.ajax({
				...
				cache: false,
				url: 'ajax/update_pg_request.php',
