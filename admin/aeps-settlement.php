    ...
    <ul class="navbar-nav ml-auto">
	  ...
	  <li class="nav-item dropdown">
        <a class="nav-link" data-toggle="dropdown" href="#">
          <i class="fas fa-wallet"></i> Virtual
          <span class="badge badge-info"><i class="fas fa-rupee-sign"></i> <span id="navVB">0.00</span></span>
        </a>
      </li>
      <li class="nav-item dropdown">
        <a class="nav-link" data-toggle="dropdown" href="#">
...
$resBank = $mysqlClass->fetchRow("wl_bank_details"," * "," where user_id = '".$USER_ID."'");
...
if(isset($_POST['addBank'])){
	...
	$data = array(
		...
		$id = $mysqlClass->insertData('wl_bank_details',$data);
		if($id>0){
	...
	if(isset($_POST['withdrawal']) && (isset($_POST['totalAmt']) && $_POST['totalAmt']>0)){ 
		...
		$retailerTran = $mysqlClass->fetchRow("wl_aeps_withdrawl", " settlement_date ", " where user_id ='".$USER_ID."'  ORDER BY  id DESC LIMIT 1");
		...
		if($transaction_gap > 120 ) {
			...
			if(isset($post['settlement_type']) && $post['settlement_type']=="wallet"){ 
				$totalAmt = $mysqlClass->fetchRow("aeps_info"," sum(amount) as totalWithdrawalAmt "," where wl_id = '".$WL_ID."' and id in (".implode(",",$post['settlementId']).")  and settlement_wl = 'PENDING' and status = 'SUCCESS' and txntype = 'WAP' ");
				...
				if($withdrawal_Amount == (float) $post['totalAmt']){
					...
					$admin_avlVbal = $mysqlClass->mysqlQuery("SELECT balance FROM `admin` WHERE `id`='" . $ADMIN_ID . "'  FOR UPDATE ")->fetch(PDO::FETCH_ASSOC);	
					
					$admin_v_bal  = $admin_avlVbal['balance'];
					$amount	= $post['amount'];
					
					if ( $admin_v_bal > $withdrawal_Amount ){ 
						
						/* *** Update AEPS Transactions settlement (aeps_info) *** */
						foreach($post['settlementId'] as $row){						
							$Udata = array(
								'settlement_wl'		=> 'SUCCESS',				
								'wl_group_id'		=> $group_id
							);
							$mysqlClass->updateData('aeps_info',$Udata," where wl_id = '".$WL_ID."' and id = '".$row."' ");
						}
						...
						$id = $mysqlClass->insertData('wl_aeps_withdrawl',$data);
						
						/* *** Get User Current Balance (Opening Balance) *** */
						
						$rt_avlVbal = $userClass->check_user_balance($WL_ID," FOR UPDATE");
						$rt_closingBal = $userClass->update_wallet_balance_add_amount($WL_ID,$withdrawal_Amount);
						
						$dataValueRt = array(
						'ret_dest_wl_admin_id' 		=> $WL_ID,
						'transaction_id' 			=> $transaction_id,
						...
						$last_tran = $mysqlClass->insertData(" wl_trans ", $dataValueRt);
						
						/* *** Get User Current AEPS Balance (Opening Balance) *** */
						$aeps_avlbal = $userClass->check_user_aeps_wallet_balance($WL_ID," FOR UPDATE");
						
						$updateAmt = (float) $aeps_avlbal -  $withdrawal_Amount;
						$userClass->update_aeps_wallet_balance($WL_ID,$updateAmt);
						
						if($last_tran > 0){
							
							$mysqlClass->mysqlQuery("UPDATE `admin` set balance = (balance - $withdrawal_Amount) WHERE  id = '".$ADMIN_ID."' ");
							...
							$dataValue = array(
								...
								'agent_trid' 		=> $transaction_id,
								'wl_id' 			=> $WL_ID,
								'opening_balance' 	=> $admin_v_bal,
								'deposits' 			=> 0,
								'withdrawl' 		=> $withdrawal_Amount,
								'balance' 			=> ($admin_v_bal - $withdrawal_Amount),
							...
							$mysqlClass->insertData(" admin_trans ", $dataValue);
			...
			echo '<script type="text/javascript"> alert("Sorry!!. You are doing duplicate transaction!!");</script>';
		}
	}
			...
			<div class="row">					  
				<div class="col-md-5">					  
					<div class="row">					  
					  ...
					  <div class="col-6 form-group">
						<button type="submit" name="withdrawal" class="btn btn-primary">Withdrawal</button>
					  </div>
					</div>
				</div>
...
$sql = "select * from aeps_info where `wl_id`='".$USER_ID."' and settlement_wl = 'PENDING' and status = 'SUCCESS' and txntype = 'WAP' and amount > 0 ";
