...
$post = $helpers->clearSlashes($_POST); 
	if(isset($_POST['amount']) && $_POST['amount'] > 0 && isset($_POST['user']) && isset($_POST['userType'])){
		$mysqlClass->mysqlQuery("START TRANSACTION");
			...
			$dr_user_avlVbal = $userClass->check_user_balance($dr_UserId," FOR UPDATE");
						
			$transaction_id = $helpers->transaction_id_generator("CR",4);
			
			if ( $dr_user_avlVbal > $amount ) {
				
				if($post['userType']=="DISTRIBUTOR"){					
					$dist_avlVbal = $dr_user_avlVbal;
					$dist_closingBal = $userClass->update_wallet_balance_deduct_amount($dr_UserId,$amount);
				...
				if($post['userType']=="RETAILER"){
					$rt_avlVbal = $dr_user_avlVbal;
					$rt_closingBal = $userClass->update_wallet_balance_deduct_amount($dr_UserId,$amount);
				...
				$mysqlClass->mysqlQuery("COMMIT");
					...
					$helpers->redirect_page("debit-from-user");
				} else {
				...
				$msg  = $helpers->alert_message("User have insufficient balance (Balance:  $dr_user_avlVbal).","alert-danger");
				$mysqlClass->mysqlQuery("COMMIT");
			}
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Debit From User</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">Debit From User</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
		...
		<div class="row justify-content-center">
          ...
          <div class="card card-warning">
            <div class="card-header">
              <h3 class="card-title">Debit From User</h3>
            ...
            <div class="card-body row">
			  ...
			  <div class="card-footer">
				<button type="submit" name="debit"  class="btn btn-primary">Debit</button>
			  </div>
			</form>
            <!-- /.card-body -->
          </div>
...
$(document).ready(function(){
    ...
    $('#debitForm').on('submit', function(e){
		...
		var textMessage = "<table class='table'> <tr><th>User:</th><td>" + user + "<td></tr> <tr><th>Amount:</th><td>Rs. " + amount + "<td></tr> <tr><th colspan='2' style='text-align:center;color:green'>" + amountinword + "</th></tr></table>";
			...
			Swal.fire({
			  title: "Are you sure Debit ?",
			  html: textMessage,
			  type: "warning",
			  showCancelButton: true,
			  confirmButtonColor: "#DD6B55",
			  confirmButtonText: "Debit Now",
