...
***************/
	...
	$tranDetails = $mysqlClass->mysqlQuery("Select agent_trid,user_id,wl_id,txntype,amount from aeps_info where agent_trid = '".$agent_trid."' and status = 'PENDING'")->fetch(PDO::FETCH_ASSOC);
	if(isset($tranDetails['agent_trid']) && !empty($tranDetails['agent_trid'])){
		...
		if(round((float)$amount,3) != round((float)$tranDetails['amount'],3)){
			$helpers->errorResponse(" Amount mismatch error.");
		}
	} else {
	...
	//// Transaction start ////	
		if(!empty($USER_ID) && !empty($WL_ID) && !empty($txntype) && $txntype == 'WAP' && $txn_status == 'SUCCESS'){
			...
			$aeps_lastid = $mysqlClass->updateData(" aeps_info ", $valueAEPS, " where agent_trid = '".$agent_trid."' and status = 'PENDING' and user_id = '".$USER_ID."'" );
			if($aeps_lastid > 0){ 
